...
const Field = ({ label, children, hint }) => (
  <div className="mb-5">
    ...
    {children}
    {hint && <div className="text-[10px] text-zinc-600 mt-1.5 italic">{hint}</div>}
  </div>
);
...
const Toggle = ({ on, onChange }) => (
  ...
  >
    <span
      className={`inline-block h-4 w-4 rounded-full bg-zinc-100 transition-transform ${
...
const SelectInput = ({ value, onChange, children }) => (
  <select
    ...
    className="w-full bg-zinc-950/40 border border-zinc-800 rounded px-3 py-2 text-xs text-zinc-200 focus:outline-none focus:border-zinc-600 transition-colors"
  >
    {children}
  </select>
);
...
export default function SettingsPanel({ open, onClose, settings, updateSetting, resetSettings }) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <aside
        className="fixed top-0 right-0 bottom-0 w-full max-w-sm bg-zinc-950 border-l border-zinc-800/80 z-50 flex flex-col shadow-2xl"
        role="dialog"
        aria-label="Settings"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800/80">
          <div>
            <h2 className="font-display text-xl text-zinc-100 leading-none">Settings</h2>
            <div className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-mono mt-1">stored locally</div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded border border-zinc-700 hover:border-zinc-500 text-zinc-400 hover:text-zinc-200 transition-colors"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          {/* Theme */}
          <Field label="Theme" hint="Light / system will activate once the styling pass lands.">
          ...
          {/* Accent palette */}
          <Field label="Accent palette" hint="Applied site-wide in a later commit.">
            ...
            <div className="grid grid-cols-3 gap-2">
              {DENSITY_OPTIONS.map((d) => {
                ...
                return (
                  ...
                  >
                    <div className="text-[11px] text-zinc-100 font-medium">{d.label}</div>
          ...
          <Field label="Auto-refresh">
            <div className="flex items-center justify-between">
              <span className="text-xs text-zinc-300">Re-fetch data.json automatically</span>
              <Toggle
          ...
          <Field label="Completed tasks">
            <div className="flex items-center justify-between">
              <span className="text-xs text-zinc-300">Hide tasks marked Done</span>
              <Toggle
        ...
        {/* Footer */}
        <div className="border-t border-zinc-800/80 px-5 py-3 flex items-center justify-between">
          <button
            onClick={resetSettings}
            className="text-[11px] text-zinc-500 hover:text-zinc-300 transition-colors uppercase tracking-widest font-medium"
          >
            Reset defaults
          </button>
          <span className="text-[10px] text-zinc-600 font-mono">v1</span>
        </div>
      </aside>
    </>
  );
